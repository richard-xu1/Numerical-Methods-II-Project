...
def main(np.int h):
    ...
    cdef np.ndarray vtp
    cdef np.ndarray vtj
    cdef np.ndarray conv_std = np.zeros(100)
    cdef np.ndarray timestamp = np.zeros(100)
    ...
    if case ==3:
        ...
        dt= 2*dt
    while t < T:
        if case == 1 and counter%4 == 0 and counter < 800 and counter >400:
            conv_std[(counter / 4)-101] = v_k[p_index]
            timestamp[(counter / 4)-101] = t
            # print "timestamp is" + str(t)
        elif case ==2 and counter%2 == 0 and counter <400 and counter >200:
            conv_std[(counter / 2) -101 ]= v_k[p_index]
            timestamp[(counter / 2)-101] = t
            # print "timestamp is" + str(t)
        elif case ==3 and counter <200 and counter >100:
            conv_std[counter-101] = v_k[p_index]
            timestamp[counter-101] = t
            # print "timestamp is" + str(t)
        counter = counter+1
    ...
    plt.show()
    # print "conv_std is" +str(conv_std)
    return conv_std, timestamp